; Shangcoin GUI Wallet Beta 2 Installer for Windows
; Copyright (c) 2014-2017, The Shangcoin Project
; See LICENSE

[Setup]
AppName=Shangcoin GUI Wallet
AppVersion=0.10.3.1
DefaultDirName={pf}\Shangcoin GUI Wallet
DefaultGroupName=Shangcoin GUI Wallet
UninstallDisplayIcon={app}\shangcoin-wallet-gui.exe
...
; Shangcoin GUI wallet
Source: "bin\shangcoin-wallet-gui.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Shangcoin GUI wallet log file
...
Source: "shangcoin-wallet-gui.log"; DestDir: "{app}"; Flags: comparetimestamp; Permissions: users-modify

; Shangcoin CLI wallet
Source: "bin\shangcoin-wallet-cli.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Shangcoin wallet RPC interface implementation
Source: "bin\shangcoin-wallet-rpc.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Shangcoin daemon
Source: "bin\shangcoind.exe"; DestDir: "{app}"; Flags: comparetimestamp

; Shangcoin daemon wrapped in a batch file that stops before the text window closes, to see any error messages
Source: "shangcoin-daemon.bat"; DestDir: "{app}"; Flags: comparetimestamp;

; Shangcoin blockchain utilities
Source: "bin\shangcoin-blockchain-export.exe"; DestDir: "{app}"; Flags: comparetimestamp
Source: "bin\shangcoin-blockchain-import.exe"; DestDir: "{app}"; Flags: comparetimestamp
Source: "bin\shangcoin-utils-deserialize.exe"; DestDir: "{app}"; Flags: comparetimestamp
...
; Filename: "{app}\shangcoin-wallet-gui.exe"; Description: "Run GUI Wallet now"; Flags: postinstall nowait skipifsilent
...
begin
  ...
  blockChainDefaultDir := ExpandConstant('{commonappdata}\shangcoin');
  s := 'The default folder to store the Shangcoin blockchain is ' + blockChainDefaultDir;
...
begin
  Result := BlockChainDir('') + '\shangcoin.log';
...
begin
  if CurStep = ssPostInstall then begin
    // Re-build "shangcoin-daemon.bat" according to actual install and blockchain directory used
    SetArrayLength(s, 3);
    s[0] := 'REM Execute the Shangcoin daemon and then stay with window open after it exits';
    s[1] := '"' + ExpandConstant('{app}\shangcoind.exe') + '" ' + DaemonFlags('');
    s[2] := 'PAUSE';
    SaveStringsToFile(ExpandConstant('{app}\shangcoin-daemon.bat'), s, false);
  end;
end;
...
[Icons]
; Icons in the "Shangcoin GUI Wallet" program group
; Windows will almost always display icons in alphabetical order, per level, so specify the text accordingly
Name: "{group}\GUI Wallet"; Filename: "{app}\shangcoin-wallet-gui.exe"
...
Name: "{group}\Utilities\Shangcoin Daemon"; Filename: "{app}\shangcoind.exe"; Parameters: {code:DaemonFlags}
Name: "{group}\Utilities\Read Me"; Filename: "{app}\ReadMe.htm"
Name: "{group}\Utilities\Textual (CLI) Wallet"; Filename: "{app}\shangcoin-wallet-cli.exe"
...
Name: "{group}\Utilities\x (Check Default Wallet Folder)"; Filename: "{win}\Explorer.exe"; Parameters: "{userdocs}\Shangcoin\wallets"
Name: "{group}\Utilities\x (Check GUI Wallet Log)"; Filename: "Notepad"; Parameters: "{app}\shangcoin-wallet-gui.log"
Name: "{group}\Utilities\x (Try Daemon, Exit Confirm)"; Filename: "{app}\shangcoin-daemon.bat"
Name: "{group}\Utilities\x (Try GUI Wallet Low Graphics Mode)"; Filename: "{app}\start-low-graphics-mode.bat"
Name: "{group}\Utilities\x (Try Kill Daemon)"; Filename: "Taskkill.exe"; Parameters: "/IM shangcoind.exe /T /F"

; Desktop icons, optional with the help of the "Task" section
Name: "{userdesktop}\GUI Wallet"; Filename: "{app}\shangcoin-wallet-gui.exe"; Tasks: desktopicon
...
Root: HKCU; Subkey: "Software\shangcoin-project"; Flags: uninsdeletekeyifempty
Root: HKCU; Subkey: "Software\shangcoin-project\shangcoin-wallet-gui"; Flags: uninsdeletekey
Root: HKCU; Subkey: "Software\shangcoin-project\shangcoin-wallet-gui"; ValueType: string; ValueName: "daemonFlags"; ValueData: {code:DaemonFlags};
