...
SHANGCOIN_DIR=shangcoin
SHANGCOIND_EXEC=shangcoind
...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/shangcoin-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    SHANGCOIND_EXEC=shangcoind.exe
fi
...
get_tag
popd
echo "var GUI_SHANGCOIN_VERSION = \"$TAGNAME\"" >> version.js

cd build
qmake ../shangcoin-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit

# Copy shangcoind to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$SHANGCOIN_DIR/bin/$SHANGCOIND_EXEC $BIN_PATH
fi
