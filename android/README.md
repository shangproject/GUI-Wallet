...
# Base environnement
        ...
        cd shangcoin/utils/build_scripts
        docker build -f android32.Dockerfile -t shangcoin-android .
        cd ..

# Build GUI

        cd android/docker
        docker build -t shangcoin-gui-android .
        docker create -it --name shangcoin-gui-android shangcoin-gui-android bash

# Get the apk

        docker cp shangcoin-gui-android:/opt/android/shangcoin-wallet-gui/build/release/bin/bin/QtApp-debug.apk .
...
- Using ADB (Android debugger bridge) : 
  ...
  The only place where we are allowed to play is `/data/local/tmp`. So : 
        ...
        adb  push /opt/android/shangcoin-wallet-gui/build/release/bin/bin/QtApp-debug.apk /data/local/tmp
