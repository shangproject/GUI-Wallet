...
#include "wallet/wallet2_api.h" // we need to have an access to the Shangcoin::Wallet::Status enum here;
...
public:
    ...
    enum Status {
        Status_Ok       = Shangcoin::Wallet::Status_Ok,
        Status_Error    = Shangcoin::Wallet::Status_Error,
        Status_Critical = Shangcoin::Wallet::Status_Critical
    };

    Q_ENUM(Status)

    enum ConnectionStatus {
        ConnectionStatus_Connected       = Shangcoin::Wallet::ConnectionStatus_Connected,
        ConnectionStatus_Disconnected    = Shangcoin::Wallet::ConnectionStatus_Disconnected,
        ConnectionStatus_WrongVersion    = Shangcoin::Wallet::ConnectionStatus_WrongVersion
    };
...
private:
    ...
    Wallet(Shangcoin::Wallet *w, QObject * parent = 0);
    ~Wallet();
private:
