...
class WalletListenerImpl : public  Shangcoin::WalletListener
{
public:
...
{
    QFuture<Shangcoin::Wallet::ConnectionStatus> future = QtConcurrent::run(m_walletImpl, &Shangcoin::Wallet::connected);
    QFutureWatcher<Shangcoin::Wallet::ConnectionStatus> *connectionWatcher = new QFutureWatcher<Shangcoin::Wallet::ConnectionStatus>();

    connect(connectionWatcher, &QFutureWatcher<Shangcoin::Wallet::ConnectionStatus>::finished, [=]() {
        QFuture<Shangcoin::Wallet::ConnectionStatus> future = connectionWatcher->future();
...
{
    Shangcoin::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), amount,
                static_cast<Shangcoin::PendingTransaction::Priority>(priority));
...
{
    Shangcoin::PendingTransaction * ptImpl = m_walletImpl->createTransaction(
                dst_addr.toStdString(), payment_id.toStdString(), Shangcoin::optional<uint64_t>(),
                static_cast<Shangcoin::PendingTransaction::Priority>(priority));
...
UnsignedTransaction * Wallet::loadTxFile(const QString &fileName)
{
    ...
    Shangcoin::UnsignedTransaction * ptImpl = m_walletImpl->loadUnsignedTx(fileName.toStdString());
...
{
    return QString::fromStdString(Shangcoin::Wallet::genPaymentId());
}
...
{
    return QString::fromStdString(m_walletImpl->getDefaultDataDir()) + "/shangcoin.log";
}

QString Wallet::getWalletLogPath() const
{
    return QCoreApplication::applicationDirPath() + "/shangcoin-wallet-gui.log";
}

Wallet::Wallet(Shangcoin::Wallet *w, QObject *parent)
    : QObject(parent)
    , m_walletImpl(w)
    , m_history(nullptr)
...
Wallet::~Wallet()
{
    ...
    delete m_history;
    m_history = NULL;
    //Shangcoin::WalletManagerFactory::getWalletManager()->closeWallet(m_walletImpl);
    delete m_walletImpl;
    m_walletImpl = NULL;
