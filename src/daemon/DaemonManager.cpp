...
{
    // prepare command line arguments and pass to shangcoind
    ...
    qDebug() << "starting shangcoind " + m_shangcoind;
    ...
    // Start shangcoind
    bool started = m_daemon->startDetached(m_shangcoind, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(testnet)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM shangcoind.exe");
#else
                QProcess::execute("pkill shangcoind");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./shangcoind status` returns BUSY when syncing.
...
bool DaemonManager::sendCommand(const QString &cmd,bool testnet, QString &message) const
{
    ...
    p.start(m_shangcoind, external_cmd);
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to shangcoind
#ifdef Q_OS_WIN
    m_shangcoind = QApplication::applicationDirPath() + "/shangcoind.exe";
#elif defined(Q_OS_UNIX)
    m_shangcoind = QApplication::applicationDirPath() + "/shangcoind";
#endif

    if (m_shangcoind.length() == 0) {
